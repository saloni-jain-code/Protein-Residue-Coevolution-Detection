...
single_double_changes_count <- function(mat) {
	...
	grid[,3:4]<- t(apply(grid,1,function(x) {
		i <- as.numeric(unlist(x[1]))
		j <- as.numeric(unlist(x[2]))
		
		v <- rep(0,times=2)

		# The following commands counts changes in each branch independently, and then sums all of them. This is different to the nodes-based calculation		
		v[1] <- v[1] + sum(((mat[,i]==0 & mat[,j]==1) | (mat[,i]==1 & mat[,j]==0))+0.0)
		v[1] <- v[1] + sum(((mat[,i+n]==0 & mat[,j+n]==2) | (mat[,i+n]==2 & mat[,j+n]==0))+0.0)
				
		v[2] <- v[2] + sum((mat[,i]==1 & mat[,j]==1)+0.0)		
		v[2] <- v[2] + sum((mat[,i+n]==2 & mat[,j+n]==2)+0.0)
		#End
...
# The branches method counts "left" and "right" branch changes individually
# For the purposes of detecting coevolution these are equivalent and so 
# edit_branches_matrix <- function(df) {
#   df2 <- df
#   df[df == "2"] <- 0
#   df[df == "3"] <- 1
#   df2[df2 == "1"] <- 0
...
edit_branches_matrix <- function(df) {
  ...
  m <- as.matrix(df)
  storage.mode(m) <- "integer"       # make sure we have integers, not factors

  max_bit <- if (all(m == 0)) 0 else floor(log2(max(m)))
  out <- vector("list", max_bit + 1)

  for (bit in 0:max_bit) {
    mask <- bitwShiftL(1L, bit)      # 1, 2, 4, 8, …
    slice <- (m & mask) != 0L        # TRUE where this bit is set
    out[[bit + 1]] <- slice + 0L     # convert logical to {0,1}
  }

  do.call(rbind, out)                # stack the row blocks
}
...
# Read the branches matrix and edit for use
branches_matrix <- read.delim2(ifile, header = FALSE, sep = " ")
branches_matrix <- edit_branches_matrix(branches_matrix)

# output from java program needs matrix transposition
mps_changes_branches_matrix <- t(as.matrix(branches_matrix))

nrow <- nrow(mps_changes_branches_matrix)
ncol <- ncol(mps_changes_branches_matrix)
rownames(mps_changes_branches_matrix) <- seq(1,nrow,by=1)
colnames(mps_changes_branches_matrix) <- seq(1,ncol,by=1)

mps_all_df <- single_double_changes_count(mps_changes_branches_matrix)

nresidues <- dim(mps_changes_branches_matrix)[2]/2
