...
do
    ...
    do
        data_dir="${letter}_${num_sequences}_aces"
        ...
        do
            ...
            mkdir outputs
            mkdir outputs/${data_dir}_0.05
            mkdir outputs/${data_dir}_0.05/$statistical_model
            ...
            echo "OUTPUT_FILE outputs/${data_dir}_0.05/$statistical_model/mps_1aoeA" >> 1aoeA.ctl
            ...
            Rscript --vanilla scripts/covariation_branches_model.R outputs/${data_dir}_0.05/$statistical_model/mps_1aoeA.binary_branches.tab outputs/${data_dir}_0.05/$statistical_model/1aoeA_branches 0.05 $statistical_model

            # Run the nodes method on a single statistical model (3 = Box-Cox)
            Rscript --vanilla scripts/covariation_nodes_model.R outputs/${data_dir}_0.05/$statistical_model/mps_1aoeA.binary_nodes.tab outputs/${data_dir}_0.05/$statistical_model/1aoeA_nodes 0.05 $statistical_model

            # Extract only overlapping predictions and remove the header line
            comm -12 <(cut -f1-2 -d ' ' outputs/${data_dir}_0.05/$statistical_model/1aoeA_branches_predictions.txt | sort) <(cut -f1-2 -d ' ' outputs/${data_dir}_0.05/$statistical_model/1aoeA_nodes_predictions.txt | sort) | awk '$1 ~ /^[0-9]+$/ {print $1"\t"$2}' > outputs/${data_dir}_0.05/$statistical_model/1aoeA_overlap_predictions.txt
            ...
            echo "MSA_ALIGNMENT_FILE ../aCES_data/$data_dir/1aoeA.phy" > 1aoeA.ctl
            echo "TREE_FILE ../aCES_data/$data_dir/1aoeA.txt" >> 1aoeA.ctl
            echo "OUTPUT_FILE outputs/${data_dir}_0.05/$statistical_model/mps2_1aoeA" >> 1aoeA.ctl
            echo "PAIRS_FILE outputs/${data_dir}_0.05/$statistical_model/1aoeA_overlap_predictions.txt" >> 1aoeA.ctl
            ...
            Rscript --vanilla scripts/gap_filter.R data/1aoeA.phy outputs/${data_dir}_0.05/$statistical_model/1aoeA_overlap_predictions.txt outputs/${data_dir}_0.05/$statistical_model/1aoeA_filtered_predictions.tsv
        done
